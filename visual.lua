--[[
    MOON v2.4 — VISUAL MODULE (FULL)
    Aurora gradient GUI / notifications / HUD / binds / rage / sounds
]]
...
function Visual.Notify(title, text, dur, col)
    ...
    ns.Parent = nf
    local nsG = Instance.new("UIGradient")
...
function Visual.Ripple(btn, x, y)
    ...
    g.Color = Visual.getAuroraSequence(os.clock())
...
for _, n in ipairs({"SakuraGUI_v19", "SakuraGUI_v18", "SakuraGUI", "SakuraGUI_v20", "MoonGUI_Main", "MoonGUI", "MoonOverlay", "MoonLoad"}) do
...
for _, v in ipairs(Lighting:GetChildren()) do
    if v.Name:find("SakuraMainBlur") or v.Name:find("MoonBlur") or v.Name:find("MoonLoadBlur") then
...
local wmSt = Instance.new("UIStroke")
wmSt.Thickness = 1
wmSt.Transparency = 0.2
wmSt.Parent = wm
local wmStGrad = Instance.new("UIGradient")
wmStGrad.Parent = wmSt
...
function Visual.showLoadingScreen()
    ...
    local loadDim = Instance.new("Frame")
    ...
    cSt.Transparency = 1
    cSt.Parent = card
    local cStGrad = Instance.new("UIGradient")
    cStGrad.Parent = cSt
    ...
    local lBarGrad = Instance.new("UIGradient")
    lBarGrad.Parent = lBarFill
...
local bfSt = Instance.new("UIStroke")
bfSt.Thickness = 1
bfSt.Transparency = 0.25
bfSt.Parent = bindsFrame
local bfStGrad = Instance.new("UIGradient")
bfStGrad.Parent = bfSt
...
bfTitle.ZIndex = 101
bfTitle.Parent = bindsFrame

local bfContent = Instance.new("Frame")
...
function Visual.updateActiveBinds(CFG, C, aimActive, tgtActive, spinActive)
    for _, v in ipairs(bfContent:GetChildren()) do
        if v:IsA("Frame") then
            v:Destroy()
        end
    end
...
local hudSt = Instance.new("UIStroke")
hudSt.Thickness = 1
hudSt.Transparency = 0.4
hudSt.Parent = hudFrame
local hudStGrad = Instance.new("UIGradient")
hudStGrad.Parent = hudSt
Visual._hudStGrad = hudStGrad

local hudLayout = Instance.new("UIListLayout")
hudLayout.Padding = UDim.new(0, 2)
hudLayout.Parent = hudFrame
local hp2 = Instance.new("UIPadding")
...
gSt.Parent = W
local gStGrad = Instance.new("UIGradient")
gStGrad.Parent = gSt
...
local auroraBg = Instance.new("Frame")
...
auroraBg.ZIndex = 10
auroraBg.Parent = W
local auroraBgGrad = Instance.new("UIGradient")
auroraBgGrad.Rotation = 25
auroraBgGrad.Parent = auroraBg
...
local moonBtn = Instance.new("Frame")
...
local moonBtnGrad = Instance.new("UIGradient")
moonBtnGrad.Parent = moonBtn
Visual._moonBtnGrad = moonBtnGrad

local moonBtnTxt = Instance.new("TextLabel")
...
local guiFPS = Instance.new("TextLabel")
guiFPS.Text = "60"
...
local guiKills = Instance.new("TextLabel")
guiKills.Text = "0 kills"
guiKills.Size = UDim2.new(0, 55, 0, 14)
guiKills.Position = UDim2.new(0, 214, 0.5, -7)
...
local minBtn, minBtnG = mkTBtn("—", -74, function()
    isMin = not isMin
    TS:Create(W, TweenInfo.new(0.35, Enum.EasingStyle.Quint), {
...
local tabIndGrad = Instance.new("UIGradient")
tabIndGrad.Parent = tabInd
Visual._tabIndGrad = tabIndGrad
Visual.tabInd = tabInd

local tabCon = Instance.new("Frame")
...
local function bindButtonSound(btn, kind)
    btn.MouseButton1Click:Connect(function()
        if Visual._uiSoundsEnabled then
            Visual.PlaySound(kind or "Click", 0.22, 1)
        end
    end)
end
...
function Visual.makeSection(pi, tx, C)
    ...
    local lnG = Instance.new("UIGradient")
...
function Visual.makeToggle(pi, tx, k, CFG, C, cb)
    ...
    local plG = Instance.new("UIGradient")
...
function Visual.makeSlider(pi, tx, k, mn, mx, st, CFG, C, allConnections)
    ...
    local flG = Instance.new("UIGradient")
    ...
    local knG = Instance.new("UIGradient")
...
local rageSt = Instance.new("UIStroke")
rageSt.Color = Color3.fromRGB(180, 30, 30)
rageSt.Thickness = 1.5
rageSt.Transparency = 0.25
rageSt.Parent = rageGui

local rageTB = Instance.new("Frame")
...
local rContent = Instance.new("ScrollingFrame")
...
rContent.Parent = rageGui

local rLayout = Instance.new("UIListLayout")
rLayout.Padding = UDim.new(0, 4)
rLayout.SortOrder = Enum.SortOrder.LayoutOrder
rLayout.Parent = rContent
...
function Visual.updateAnimations(dt, globalT)
    ...
    auroraBgGrad.Color = aSeq
    gStGrad.Color = aSeq
    tLGrad.Color = aSeq
    tabIndGrad.Color = aSeq
    hudStGrad.Color = aSeq
    moonBtnGrad.Color = aSeq
    if closeBtnG then closeBtnG.Color = aSeq end
    if minBtnG then minBtnG.Color = aSeq end
